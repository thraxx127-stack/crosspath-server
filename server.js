...
var io = new Server(server, { cors: { origin: '*', methods: ['GET','POST'], credentials: false }, transports: ['websocket','polling'] });
...
app.get('/health', function(req, res) { res.json({ status: 'ok', queue: queue.length, sessions: sessions.size }); });

function createSession(id1, id2) {
  var s1 = io.sockets.sockets.get(id1);
  var s2 = io.sockets.sockets.get(id2);
  if (!s1 || !s2) { if (s1) { s1.data.status = 'queued'; queue.push(id1); } if (s2) { s2.data.status = 'queued'; queue.push(id2); } return; }
  ...
  s1.data.status = 'in_session'; s2.data.status = 'in_session';
  s1.data.roomId = roomId; s2.data.roomId = roomId;
  s1.data.sparksLeft = 3; s2.data.sparksLeft = 3;
  var timer = setTimeout(function(){ endSession(roomId,'timeout'); }, SESSION_MS);
  sessions.set(roomId, { users:[id1,id2], timer:timer, startTime:startTime, endTime:endTime });
  s1.emit('matched',{ roomId:roomId, startTime:startTime, duration:SESSION_MS, partnerUid:s2.data.uid || null });
  s2.emit('matched',{ roomId:roomId, startTime:startTime, duration:SESSION_MS, partnerUid:s1.data.uid || null });
}
...
function endSession(roomId, reason) {
  ...
  for (var i=0;i<session.users.length;i++) {
    var s = io.sockets.sockets.get(session.users[i]);
    if (s) { s.leave(roomId); s.data.status='idle'; s.data.roomId=null; }
  }
  sessions.delete(roomId);
  if (reason !== 'timeout') { return; }
  var rTimer = setTimeout(function(){ resolveReaction(roomId,'cross'); },12000);
  reactionWindows.set(roomId,{ sockets:session.users, votes:{}, timer:rTimer });
  for (var j=0;j<session.users.length;j++) {
    var rs = io.sockets.sockets.get(session.users[j]);
...
function resolveReaction(roomId, result) {
  ...
  for (var i=0;i<rw.sockets.length;i++) {
    ...
    s.emit('reaction_result',{ result:result, partnerUid:pUid });
  }
}

function tryMatch() {
  queue = queue.filter(function(id){ var s = io.sockets.sockets.get(id); return s && s.data.status === 'queued'; });
  if (queue.length >= 2) { var id1 = queue.shift(); var id2 = queue.shift(); createSession(id1,id2); }
}
...
io.on('connection', function(socket) {
  ...
  socket.on('send_spark', function() {
    ...
    session.timer = setTimeout(function(){ endSession(roomId,'timeout'); }, remaining);
    io.to(roomId).emit('spark_applied',{ senderId:socket.id, newEndTime:session.endTime, senderSparksLeft:socket.data.sparksLeft });
  });
  ...
  socket.on('send_reaction', function(data) {
    ...
    var keys = Object.keys(rw.votes);
    var hasCross = keys.some(function(k){ return rw.votes[k] === 'cross'; });
    if (hasCross) { resolveReaction(roomId,'cross'); return; }
    if (keys.length === rw.sockets.length) { resolveReaction(roomId,'mutual_flame'); }
  });
  ...
  socket.on('send_message', function(data) {
    ...
    if (!session || session.users.indexOf(socket.id) === -1) { return; }
    var payload = { id: Date.now() + '_' + Math.random().toString(36).slice(2,6), senderId:socket.id, message:data.message.trim().slice(0,500), timestamp:Date.now() };
